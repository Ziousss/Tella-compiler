...
SemanticType assignAnalyser(ASTnode *assignment, SemContext *context){
    ...
    if(target == NULL){
        printf("Identifier %s on the left side of the assignment line %ld file %s is not defined yet.\n", assignment->data.assign.target, assignment->line, assignment->fileName);
        context->error_count++;
        return SEM_ERROR;
    }
    
    if(target->kind != SEM_VAR && target->kind != SEM_PARAM){
        printf("Left side of assignment at line %ld file %s is not assignable.\n", assignment->line, assignment->fileName);
        context->error_count++;
        return SEM_ERROR;
    }
    ...
    if(compSizeTInt(target_type, value_type) == 1){
        if(!canConvert(target_type, value_type, assignment->data.assign.value)){
            printf("Invalid assignment on line %ld file %s: cannot assign %s to %s\n", assignment->line, assignment->fileName, fromSemToString(value_type), fromSemToString(target_type));
            context->error_count++;
            return SEM_ERROR;
        }
        ...
        if(target_type == SEM_SIZET && value_type == SEM_INT){
            if(assignment->data.assign.value->ast_type == AST_NUMBER && assignment->data.assign.value->data.int_literal.value < 0){
                printf("Warning: assigning negative literal to size_t on line %ld file %s\n", assignment->line, assignment->fileName);
            }
        }
        return value_type;
    }
    
    printf("Type mismatch on assignment line %ld file %s: %s vs %s\n", assignment->line, assignment->fileName, fromSemToString(target_type), fromSemToString(value_type));
    context->error_count++;
    return SEM_ERROR;
}